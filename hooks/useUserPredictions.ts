    ...
    try {
      ...
      const handleOffsetCalculated = (
        ...
        if (frozenOffsetRef.current === null && resyncPoint) {
          ...
          // Q3-A: notify the caller so it can persist the community offset to
          // user_offsets (DB), ensuring the value survives full app restarts.
          // This fires at most once per resync session — Q2-A already prevents
          // frozenOffsetRef.current from ever being null again after this line.
