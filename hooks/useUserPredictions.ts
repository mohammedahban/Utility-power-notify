...
//   Step 8  After the generated cycle ends, the system enters UNCERTAIN_ZONE
//           regardless of progress ratio (Q1-A), waits for Growatt to confirm,
//           then calculates the REAL offset = (generatedCycleEndMs − growattMs)
//           and saves it permanently via onOffsetCalculated (Q3-A).
//           derivedOffsetMinutes is therefore always −1 here (UNCERTAIN_ZONE
//           trigger); the real value is computed in the UNCERTAIN_ZONE exit block
//           of applyOffsetToPrediction.
//
interface CommunityTransitionResult {
  ...
  /**
   * Always −1.
   * Forces UNCERTAIN_ZONE after the generated cycle ends (Q1-A) regardless of
   * whether the interrupted cycle was >50% or <50% complete.
   * The REAL offset is calculated dynamically in the UNCERTAIN_ZONE exit block
   * and saved permanently via onOffsetCalculated (Q3-A).
   */
  readonly derivedOffsetMinutes: -1;
}
...
function computeCommunityTransition(
  ...
  // ── Step 8 (prep): Post-cycle ATC is always UNCERTAIN_ZONE ──────────────
  // derivedOffsetMinutes is always −1 regardless of whether progress was >50%
  // or <50%.  This forces UNCERTAIN_ZONE after the generated cycle ends (Q1-A).
  // The REAL offset value is calculated only when Growatt confirms, inside the
  // UNCERTAIN_ZONE exit block of applyOffsetToPrediction (Q3-A).
  // offsetType is intentionally NOT included in the return value — it is an
  // internal implementation detail used only for Step 5 duration selection.
  return {
    ...
    progressRatio,
    derivedOffsetMinutes: -1 as const,
  };
}
...
export function applyOffsetToPrediction(
  ...
  //
  //     Fires for ALL community transitions regardless of whether the original
  //     interrupted cycle was >50% or <50% complete (Q1-A).
  //
  //     Wait condition: generated cycle must have ended AND Growatt must have
  //     confirmed the transition (prediction.currentState !== currentState).
  //
  //     Real offset formula (Q2-A — same direction for both OFF and ON reports):
  //       realOffsetMinutes = (generatedCycleEndMs − growattTransitionMs) / 60_000
  //         > 0  → user's schedule ended AFTER Growatt  → POSITIVE (user behind)
  //         < 0  → user's schedule ended BEFORE Growatt → NEGATIVE (user ahead)
  //         = 0  → exact match                          → NEUTRAL
  //
  //     The calculated value is saved permanently via onOffsetCalculated (Q3-A),
  //     permanently overwriting the stored offsetMinutes.
  //     BackdatedStart = generatedCycleEndIso (user's next state started the
  //     instant the generated cycle ended — no gap, no clock drift).
  //
  // A1. Standard negative-offset user (no community transition):
  //     BackdatedStart = GrowattTransitionTime + offsetMs  (unchanged)
  //
  if (
    ...
    // ── A2: TMMS V2 community cycle exit ──────────────────────────────────
    if (communityTransition) {
      ...
      // Guard: only exit once the generated cycle has fully elapsed AND
      // Growatt has confirmed (prediction.currentState !== currentState already
      // checked by the outer if-condition).
      if (generatedCycleEndMs <= Date.now() && prediction.lastTransitionAt) {
        // ── Real offset calculation (Q2-A) ───────────────────────────────
        // Formula is identical for OFF-report and ON-report:
        //   user schedule end − Growatt confirmation time
        const growattTransitionMs = new Date(prediction.lastTransitionAt).getTime();
        const realOffsetMs        = generatedCycleEndMs - growattTransitionMs;
        const realOffsetMinutes   = realOffsetMs / 60_000;

        // ── Persist permanently (Q3-A) ────────────────────────────────────
        // Overwrite the stored offsetMinutes; stored offset was suspended
        // since the report fired (Q5-B) and is now replaced by the real value.
        onOffsetCalculated?.(realOffsetMinutes);

        // ── Anchor: user's next state started exactly when generated ended ─
        ...
        logClientAccuracy(
          ...
          realOffsetMinutes,   // log the REAL calculated offset, not the stored one
          'UNCERTAIN_ZONE',
        );
      }
      // If generated cycle has not ended yet, remain in UNCERTAIN_ZONE silently.
    } else {
      // ── A1: standard negative-offset exit (no community transition) ──────
