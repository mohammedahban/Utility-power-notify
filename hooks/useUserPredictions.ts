 ...
 *   7. Compute Offset State (POSITIVE / PENDING_NEGATIVE / NEGATIVE / NEUTRAL)
 *   8. Mark future ON slots as "Estimated (Pending Offset)" when offset
 *      is PENDING_NEGATIVE
 *   9. Subscribe to Growatt power_events to resolve PENDING_NEGATIVE
 *   10. Apply ON duration reconciliation after UNCERTAIN_ZONE
 *   11. Rebuild future predictions whenever Offset State changes
 */
...
// ── V2.2: Apply Generated ON on top of the engine's daySchedule ────────────
// Per the Personal Timeline Replacement Model: the Generated ON slot must be
// INSERTED into the daySchedule so future calculations can see it,
...
function applyGeneratedOnToSchedule(
  ...
  // Check if the schedule already contains a slot at the Generated ON start time
...
export function useUserPredictions(
  ...
  const userPrediction = useMemo((): UserPrediction | null => {
    ...
    try {
      ...
      // ── V2.2 layer: Apply Generated ON to the daySchedule ────────────────
      let v22Schedule = (engineResult.daySchedule ?? []) as ShiftedScheduleSlot[];
      v22Schedule = applyGeneratedOnToSchedule(
        v22Schedule,
        isGeneratedOnCurrent ? v22Meta.generatedOn : null,
        Date.now(),
      );
