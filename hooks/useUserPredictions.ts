/**
 * useUserPredictions — Layered Scheduling Engine  (v4.2 — R2 client write-path fix)
 *
 * Formula:
 ...
 *
 * ─────────────────────────────────────────────────────────────────────
 * v4.2 CHANGELOG (R2 fix only — see comments inline for the exact patch):
 *
 *   R2  STOP writing client reconciliation rows to prediction_accuracy_logs.
 *       Previously, logClientAccuracy() inserted rows with slot_id like
 *         "client_positive_offset_pending_offset60" or
 *         "client_uncertain_zone_offset-60"
 *       which all matched /pending_offset/i and contaminated the server's
 *       drift + consecutive-error-crisis engines. The v4.2 server filters
 *       them out at read time (R1), but the table kept growing 8× anyway.
 *       The cleanest fix (preferred per the deployment guide) is to NOT
 *       write them at all. The server already records real APPPE accuracy
 *       via power_events joins — these client rows are pure noise.
 *
 *       Implementation: logClientAccuracy() is now a NO-OP for the DB write
 *       path. It still computes errorMin/score (for local console telemetry
 *       and for an optional future diagnostic table) but never reaches
 *       supabase.from('prediction_accuracy_logs').insert(...). The two
 *       call-sites in applyOffsetToPrediction are unchanged so the rest of
 *       the reconciliation flow is untouched.
 *
 *       Migration note: existing bugged rows in prediction_accuracy_logs
 *       can be left in place — the v4.2 server's R1 filter will silently
 *       exclude them. Optionally run the SQL cleanup in the deployment
 *       guide to shrink the table:
 *
 *         DELETE FROM prediction_accuracy_logs
 *         WHERE slot_id ~ 'pending_offset';
 *
 *       Forward-only: no new bugged rows will ever be written again.
 * ─────────────────────────────────────────────────────────────────────
 *
...
// ── v4.2 R2 FIX: Client-side accuracy logger ─────────────────────────────────
//
// BEFORE (v4.1): inserted a row into prediction_accuracy_logs with slot_id
//   "client_${exitMode.toLowerCase()}_offset${offsetMinutes}" — e.g.
//   "client_positive_offset_pending_offset60" — which contaminated the
//   server's drift engine (matched /pending_offset/i and inflated the
//   dominant-error cluster around -10 min) and the consecutive-error
//   crisis trigger (3 bugged rows in a row fired false crisis → confidence
//   cap of 30%).
//
// AFTER (v4.2): NO-OP for the DB write. We still compute the local
//   accuracy numbers for console telemetry (helpful while debugging the
//   client reconciliation flow), but we never touch prediction_accuracy_logs.
//
//   The server's APPPE accuracy data already comes from joining
//   power_events with utility_predictions (see v4.2 analyze-patterns
//   loadHistory()) — client rows were always redundant noise.
//
...
async function logClientAccuracy(
  ...
  try {
    ...
    // ── R2 FIX: NO DATABASE WRITE ────────────────────────────────────────────
    //
    // The supabase insert that used to live here has been REMOVED.
    // The slot_id we wrote ("client_*_offset{N}") matched /pending_offset/i
    // and was the root cause of:
    //   • 8.34× duplicate-row inflation in prediction_accuracy_logs
    //   • False consecutive-error crisis triggers (3 bugged rows in a row)
    //   • Confidence getting capped at 30% on contaminated data
    //   • Drift offset quantizing onto the -10 min polling artifact
    //
    // The v4.2 server masks this with R1 (filter at read time), but the
    // table keeps growing. The PREFERRED fix (per deployment guide) is to
    // stop writing the bugged rows here — which is what this no-op does.
    //
    // Local telemetry only — useful while debugging, never reaches the DB.
    console.log(
      `[useUserPredictions][v4.2 R2] Client accuracy (NOT logged to DB): ` +
      `exit=${exitMode} offset=${offsetMinutes}min error=${errorMin.toFixed(1)}min score=${accuracyScore.toFixed(1)}%`,
    );

    // ── OPTIONAL: write to a separate diagnostic table ───────────────────────
    // If you want to keep client-perspective reconciliation telemetry for
    // debugging without contaminating APPPE's accuracy table, create a
    // separate table and write here:
    //
    //   CREATE TABLE client_reconciliation_diagnostics (
    //     id              BIGSERIAL PRIMARY KEY,
    //     created_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    //     exit_mode       TEXT NOT NULL,           -- 'UNCERTAIN_ZONE' | 'POSITIVE_OFFSET_PENDING'
    //     offset_minutes  INTEGER NOT NULL,
    //     predicted_iso   TIMESTAMPTZ NOT NULL,
    //     actual_iso      TIMESTAMPTZ NOT NULL,
    //     error_minutes   DOUBLE PRECISION NOT NULL,
    //     accuracy_score  DOUBLE PRECISION NOT NULL,
    //     target_state    TEXT NOT NULL            -- 'ON' | 'OFF'
    //   );
    //
    // Then uncomment the block below. The v4.2 server's R1 filter only
    // excludes rows whose slot_id matches /pending_offset/i — so writing
    // here keeps APPPE clean while still letting you inspect client
    // reconciliation behavior in isolation.
    //
    // await supabase.from('client_reconciliation_diagnostics').insert({
    //   exit_mode:      exitMode,
    //   offset_minutes: offsetMinutes,
    //   predicted_iso:  predictedTransitionIso,
    //   actual_iso:     actualTransitionIso,
...
export interface UserPrediction {
  ...
  /**
   * v4.2: raw APPPE block from the server, exposed so the UI can read
   * diagnostics like historyDiagnostics.clientRowsFiltered (R1 filter
   * count), driftQuantizationRatio, etc. Opaque on purpose — structure
   * defined server-side in analyze-patterns-v4.2.ts.
   */
  apppe?: any;
  /**
...
function findActiveSlotInRawSchedule(
  ...
  // Fallback: nearest slot of this state by start-time distance
...
export function applyOffsetToPrediction(
  ...
  if (
    ...
    if (backdatedStart) {
      ...
      if (prediction.lastTransitionAt) {
        // v4.2 R2 FIX: this call is now a no-op for the DB write path.
        // It still runs local console telemetry — see logClientAccuracy() above.
        logClientAccuracy(
          backdatedStart,
          prediction.lastTransitionAt,
  ...
  if (
    ...
    if (scheduledMs <= Date.now()) {
      ...
      // v4.2 R2 FIX: logClientAccuracy() is now a no-op for the DB write path.
      if (prediction.lastTransitionAt) {
        logClientAccuracy(
          atcState.scheduledAutoTransitionIso,
          prediction.lastTransitionAt,
  ...
  return {
    ...
    // v4.2: expose raw APPPE block so the UI can read historyDiagnostics.clientRowsFiltered
    // (R1 filter count) and other diagnostic fields without re-deriving them.
