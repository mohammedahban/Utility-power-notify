...
export interface ResyncHistoryEntry {
  ...
  reporter_username: string | null;
  /**
   * TMMS V2.1: All NEW entries will have reported_state = 'UTILITY_ON' because
   * users no longer report UTILITY_OFF (spec §1: "Remove every OFF reporting
   * logic"). The union type is kept for backward compatibility with existing
   * database records created before the V2.1 migration.
   */
...
export interface YesResyncResult {
  ...
  /**
   * The utility state that became active at effectiveTransitionAt.
   *
   * TMMS V2.1: This will always be 'UTILITY_ON' for new reports because the
   * UTILITY_OFF guard in respond() prevents any YES response from reaching this
   * result object for an OFF notification.  The union type is kept for backward
   * compatibility with any code that was written against the pre-V2.1 interface.
   */
...
export function useResyncNotifications() {
  ...
  const respond = useCallback(async (
    ...
    // ── TMMS V2.1: UTILITY_OFF confirmation guard ────────────────────────────
    // Community synchronisation now supports UTILITY_ON reports ONLY.
    // OFF transitions are computed and applied automatically by the prediction
    // engine — no recipient confirmation is ever needed or accepted.
    // This guard protects against legacy UTILITY_OFF notifications that may
    // still exist in the database from before the V2.1 migration.
    // Rule references: spec §1 "Remove every OFF confirmation logic";
    //                  spec §APPROVER LOGIC "clone Reporter's sync only for ON".
    if (response === 'yes' && notif.reported_state === 'UTILITY_OFF') {
      return {
        yesResult: null,
        error: 'بلاغات الانقطاع لم تعد مدعومة في هذا الإصدار. يتولّى التطبيق إنهاء التشغيل تلقائياً وفق التوقع المحسوب.',
      };
    }
