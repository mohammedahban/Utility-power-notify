 ...
 *         - Generated ON metadata (duration, reference, kind)
 *     These values are persisted to resync_history.
 *
 ...
 *     FIX V2.2: The query now ONLY targets rows with offset_state = 'PENDING_NEGATIVE'
 *     instead of all unresolved rows.
 *
 *  5. NOTIFICATION ENRICHMENT (V2.2 added)
 *     The notification fetch now selects all Generated ON metadata columns
 *     from utility_reports so approvers can clone the complete state:
 *         - generated_on_duration_min
 *         - generated_on_reference_iso
 *         - generated_on_reference_kind
 *
 *  6. COMMUNITY CONFIRMATION NEVER TOUCHES TIMELINE (unchanged)
...
export interface ResyncNotification {
  ...
  // ── V2.2: Generated ON metadata for approver cloning ──
...
// ── V2.2: PENDING_NEGATIVE auto-resolution ────────────────────────────────
//
...
//
// FIX V2.2: Now ONLY updates rows where offset_state = 'PENDING_NEGATIVE'
// instead of all rows with pending_resolved_at = null.
async function resolvePendingNegativeOffsets(growattOnIso: string): Promise<void> {
  ...
  // FIX V2.2: Only find PENDING_NEGATIVE rows — not all unresolved rows
  ...
  for (const row of pendingRows) {
    ...
    const finalState: OffsetState = 'NEGATIVE';
  ...
  for (const userId of userIds) {
    const userRows = pendingRows.filter(r => r.user_id === userId);
...
export function useResyncNotifications() {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    // V2.2: Enrich notifications with ALL Generated ON metadata fields
   ...
   * state including Generated ON metadata.
   */
  const respond = useCallback(async (
    ...
    if (response === 'yes') {
      ...
      // V2.2: Clone the Generated ON metadata from the notification
      // (now properly enriched in fetchNotifications)
      ...
      // Persist to resync_history — V2.2: include cloned offset data and Generated ON metadata
