...
import { fmtYemenTime, type ShiftedScheduleSlot, type ReportRecord, type TrustLevel } from './tmmsEngine';
...
function UserTimelineSimulator({ world }: { world: SimWorld }) {
  ...
  return (
    ...
    >
      ...
      <StatRow
        label="Verification Window Active"
        value={r.atc.mode === 'POSITIVE_OFFSET_PENDING' || r.atc.inValidationWindow ? 'YES' : 'no'}
        valueColor={r.atc.mode === 'POSITIVE_OFFSET_PENDING' || r.atc.inValidationWindow ? C.positive : C.textMuted}
      />
      {r.atc.statusLine && (
...
function ConfidenceConfirmationLedger({ world }: { world: SimWorld }) {
  ...
  return (
    ...
    >
      {activeReport && (
        <div style={{ marginBottom: 12, padding: 9, background: `${trustColor(activeReport.trustLevel)}12`, border: `1px solid ${trustColor(activeReport.trustLevel)}45`, borderRadius: 6 }}>
          <div style={{ fontSize: 10, color: C.textMuted, marginBottom: 4, fontWeight: 700, letterSpacing: 0.3 }}>ACTIVE REPORT (drives current generated state)</div>
          ...
          {activeReport.confirmations.length > 0 && (
            <div style={{ marginTop: 6 }}>
              {activeReport.confirmations.map((c, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: C.textSecondary, padding: '3px 0', borderTop: `1px solid ${C.border}` }}>
                  <span>#{i + 1} {c.confirmerName ?? 'confirmer'}</span>
                  <span style={{ fontFamily: C.mono }}>{c.hoursAfterReport.toFixed(1)}h after report</span>
                  <span style={{ fontFamily: C.mono, color: C.positive }}>conf→{c.confidenceScoreAfter}</span>
                </div>
              ))}
            </div>
          )}
          <div style={{ fontSize: 10, color: C.textMuted, marginTop: 8, lineHeight: 1.5 }}>
            ...
            <span style={{ color: C.generated, fontFamily: C.mono }}>{fmtClock(activeReport.originalReportAtIso)}</span> — the original report time — never any confirmation's own timestamp, no matter how many confirmations arrived or how late.
          </div>
        </div>
      )}

      <div style={{ fontSize: 10.5, color: C.textMuted, margin: '4px 0 6px', fontWeight: 700 }}>FULL PERSISTENT REPORT LEDGER (never cleared — Rule 2)</div>
      <div className="tmms-scroll" style={{ maxHeight: 240, overflowY: 'auto' }}>
        {reports.length === 0 && <div style={{ fontSize: 11, color: C.textMuted, textAlign: 'center', padding: '12px 0' }}>No reports submitted yet this session.</div>}
        {reports.map(r => (
          <div key={r.id} style={{
            display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 8px', marginBottom: 3,
            background: C.panelDeep, borderRadius: 5, border: `1px solid ${C.border}`, fontSize: 10.5,
          }}>
            <span style={{ color: stateColor(r.state), fontFamily: C.mono, fontWeight: 700, width: 32 }}>{r.state}</span>
            <span style={{ color: C.textMuted, fontFamily: C.mono, flex: 1 }}>{fmtClock(r.originalReportAtIso)}</span>
            <span style={{ color: C.textSecondary, width: 50, textAlign: 'right' }}>{r.confirmations.length}✓</span>
...
// ════════════════════════════════════════════════════════════════════════════
// This is the literal scenario set from the TMMS V2 spec document — distinct
// from the 15 built-in mechanism tests above. Covers the Master Test
// Schedule, Offset Calculation (A/B/J), Confirmation-vs-Report equivalence
// (C), Duration Selection (D/E), Generated State Completion / Zones (F/I),
// Schedule Continuity (G), Persistence (H), and — the centerpiece — the
// Community Confirmation Timestamp Rule (K).
          ...
          return (
            <div key={group} style={{ marginBottom: 6 }}>
              <div style={{
                ...
                padding: '6px 4px', fontSize: 10.5, fontWeight: 700, color: C.textSecondary,
              ...
              {scs.map(sc => {
                ...
                return (
                  <div key={sc.id} style={{ padding: '6px 4px', borderBottom: `1px solid ${C.border}` }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                      <div style={{ fontSize: 10.5, fontWeight: 600, color: C.textPrimary }}>{sc.id} — {sc.name}</div>
                      <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexShrink: 0 }}>
                        {res && <Badge color={res.pass ? C.on : C.error}>{res.pass ? 'PASS' : 'FAIL'}</Badge>}
                        {res && <button className="tmms-btn" onClick={() => onLoadWorld(res.world)} style={{ background: 'transparent', border: `1px solid ${C.border}`, borderRadius: 5, color: C.positive, fontSize: 9.5, padding: '2px 7px' }}>Inspect</button>}
                      </div>
                    </div>
                    {res && (
                      <div className="tmms-fade-in" style={{ fontSize: 9.5, color: C.textMuted, marginTop: 3, fontFamily: C.mono }}>
                        expected: {res.expected}<br />actual: {res.actual}
                      </div>
                    )}
                  </div>
    ...
    <div className="tmms-root" style={{ minHeight: '100vh', background: C.bgGradient, color: C.textPrimary, padding: 18 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18, flexWrap: 'wrap', gap: 10 }}>
        <div>
          ...
          <div style={{ fontSize: 10.5, color: C.textMuted, marginTop: 2 }}>Development tool only · drives the real engine, not a mock · 50 spec scenarios (A–K) + 15 mechanism tests</div>
        </div>
