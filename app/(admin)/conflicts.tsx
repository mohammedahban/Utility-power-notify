...
function ReviewModal({ visible, conflict, onClose, onSubmit, submitting }: {
  ...
  onClose: () => void;
  onSubmit: (conflictId: number, notes: string) => void;
  submitting: boolean;
}) {
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View style={rmStyles.overlay}>
        <View style={rmStyles.sheet}>
          ...
          <Text style={rmStyles.title}>Review Conflict #{conflict.id}</Text>

          <View style={rmStyles.statesRow}>
            <View style={[rmStyles.stateBox, { borderColor: growattColor + '55' }]}>
              <Text style={rmStyles.stateBoxLabel}>GROWATT SENSOR</Text>
              <Text style={[rmStyles.stateBoxValue, { color: growattColor }]}>
                {isOn(conflict.growatt_state) ? '⚡ Grid ON' : '🔴 Grid OFF'}
              </Text>
            </View>
            <Text style={rmStyles.vs}>≠</Text>
            <View style={[rmStyles.stateBox, { borderColor: reportedColor + '55' }]}>
              <Text style={rmStyles.stateBoxLabel}>COMMUNITY REPORT</Text>
              <Text style={[rmStyles.stateBoxValue, { color: reportedColor }]}>
                {isOn(conflict.reported_state) ? '⚡ Grid ON' : '🔴 Grid OFF'}
              </Text>
            </View>
          </View>

          <Text style={rmStyles.label}>ADMIN NOTES</Text>
          <TextInput
            style={rmStyles.input}
            value={notes}
            onChangeText={setNotes}
            placeholder="Add notes about this conflict (optional)…"
          ...
          >
            {submitting
              ...
              : <Text style={rmStyles.submitText}>✓  Mark as Reviewed</Text>
            }
          </TouchableOpacity>
...
const rmStyles = StyleSheet.create({
  ...
  title: { color: T.textPrimary, fontSize: 18, fontWeight: '800', marginBottom: 20 },
  statesRow: { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 20 },
  stateBox: { flex: 1, backgroundColor: T.elevated, borderRadius: 12, padding: 14, borderWidth: 1 },
  stateBoxLabel: { color: T.textMuted, fontSize: 9, fontWeight: '700', letterSpacing: 1.2, marginBottom: 6 },
  stateBoxValue: { fontSize: 15, fontWeight: '800' },
  vs: { color: T.warning, fontSize: 22, fontWeight: '900' },
  label: { color: T.textMuted, fontSize: 9, fontWeight: '700', letterSpacing: 1.5, marginBottom: 8 },
...
function ConflictCard({ conflict, onReview }: { conflict: Conflict; onReview: () => void }) {
  ...
  const occurredAt = conflict.estimated_transition_at
    ? new Date(conflict.estimated_transition_at).toLocaleString('en-US', {
        timeZone: 'Asia/Aden', month: 'short', day: 'numeric',
        hour: '2-digit', minute: '2-digit',
      })
    : new Date(conflict.created_at).toLocaleString('en-US', {
        timeZone: 'Asia/Aden', month: 'short', day: 'numeric',
        hour: '2-digit', minute: '2-digit',
      });
  ...
  return (
    <View style={[ccStyles.card, isReviewed && { opacity: 0.55 }]}>
      ...
      <View style={ccStyles.header}>
        <Text style={ccStyles.id}>Conflict #{conflict.id}</Text>
      ...
      <View style={ccStyles.statesRow}>
        <View style={ccStyles.stateItem}>
          <Text style={ccStyles.stateLabel}>GROWATT</Text>
          <Text style={[ccStyles.stateVal, { color: growattColor }]}>
            {isOn(conflict.growatt_state) ? '⚡ ON' : '🔴 OFF'}
          </Text>
        </View>
        <Text style={ccStyles.vsText}>≠</Text>
        <View style={ccStyles.stateItem}>
          <Text style={ccStyles.stateLabel}>REPORTED</Text>
          <Text style={[ccStyles.stateVal, { color: reportedColor }]}>
            {isOn(conflict.reported_state) ? '⚡ ON' : '🔴 OFF'}
          </Text>
        </View>
        <View style={ccStyles.metaItem}>
          <Text style={ccStyles.stateLabel}>REPORTER</Text>
          <Text style={ccStyles.metaVal}>{conflict.reporter_username ?? '—'}</Text>
        </View>
        <View style={ccStyles.metaItem}>
          <Text style={ccStyles.stateLabel}>TIME</Text>
          <Text style={ccStyles.metaVal}>{conflict.time_option ? timeLabel[conflict.time_option] ?? conflict.time_option : '—'}</Text>
        </View>
      </View>

      <Text style={ccStyles.timestamp}>Reported at {occurredAt} (Yemen)</Text>

      {conflict.notes ? (
        <View style={ccStyles.notesBox}>
          <Text style={ccStyles.notesLabel}>Admin Notes: </Text>
      ...
      {!isReviewed && (
        <TouchableOpacity style={ccStyles.reviewBtn} onPress={onReview} activeOpacity={0.85}>
          <Text style={ccStyles.reviewBtnText}>Review This Conflict</Text>
        </TouchableOpacity>
      )}
    </View>
  );
...
const ccStyles = StyleSheet.create({
  ...
  unreviewedDot: { position: 'absolute', top: 14, right: 14, width: 8, height: 8, borderRadius: 4, backgroundColor: T.warning },
  ...
  statesRow: { flexDirection: 'row', alignItems: 'center', backgroundColor: T.elevated, borderRadius: 10, padding: 10, gap: 4, marginBottom: 10 },
  ...
  timestamp: { color: T.textMuted, fontSize: 11, marginBottom: 8 },
  notesBox: { backgroundColor: T.elevated, borderRadius: 8, padding: 10, marginBottom: 10, flexDirection: 'row', flexWrap: 'wrap' },
  notesLabel: { color: T.textMuted, fontSize: 11, fontWeight: '700' },
  notesText: { color: T.textSecondary, fontSize: 11, flex: 1 },
...
export default function ConflictsScreen() {
  ...
  const fetchConflicts = useCallback(async () => {
    ...
    try {
      ...
      // Enrich with report info + reporter username
      const reportIds = [...new Set(rows.map(r => r.report_id))];
      if (reportIds.length > 0) {
        const { data: reports } = await supabase
          .from('utility_reports')
          .select('id, reporter_id, time_option, estimated_transition_at')
          .in('id', reportIds);
        ...
        const enriched = rows.map(c => {
          ...
          return {
            ...c,
            reporter_username: rep ? (usernameMap[rep.reporter_id] ?? null) : null,
            time_option: rep?.time_option ?? null,
            estimated_transition_at: rep?.estimated_transition_at ?? null,
          };
        });
      ...
      {loading && conflicts.length === 0 ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <ConflictCard
              conflict={item}
              onReview={() => setSelectedConflict(item)}
            />
          )}
          ListEmptyComponent={() => (
            <View style={styles.emptyBox}>
              ...
              <Text style={styles.emptyTitle}>
                {filter === 'pending' ? 'No Pending Conflicts' : 'No Conflicts Found'}
              </Text>
              <Text style={styles.emptySub}>
                {filter === 'pending'
                  ? 'All community reports are in agreement with Growatt sensor data.'
                  : 'Community reports and sensor data have been consistent.'}
              </Text>
            </View>
          )}
        />
...
const styles = StyleSheet.create({
  ...
  statsBar: { flexDirection: 'row', backgroundColor: T.surface, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: T.border, alignItems: 'center' },
  ...
  filterRow: { flexDirection: 'row', backgroundColor: T.surface, borderBottomWidth: 1, borderBottomColor: T.border },
