...
function PersonalStatusCard({ prediction, anchorStartIso, onRevertToGrowatt, hasSnapshot, reasoningLine }: {
  ...
  const currentSlot = (() => {
    ...
    const nowMs = Date.now();
    // POSITIVE_OFFSET_PENDING and community/user-resynced states inject the
    // current held slot at index 0 of daySchedule — same pattern already
    // used successfully in TodayTimeline. Handle them explicitly so the
    // generic isHolding flag below doesn't blank out a perfectly known slot.
    if ((atcMode === 'POSITIVE_OFFSET_PENDING' || prediction?.isResynced) && slots.length > 0) {
      return slots[0];
    }
...
function TodayTimeline({ prediction, anchorStartIso }: {
  ...
  // Community-confirmed / user-reported resyncs also inject the current held
  // slot at index 0 of daySchedule — same pattern as POSITIVE_OFFSET_PENDING.
  const isResyncedHold = !!prediction?.isResynced;

  const activeIdx = (() => {
    // Synthetic lingering slot is always at index 0 when POSITIVE_OFFSET_PENDING
    // or when the state was just resynced via a community/user report.
    if ((isPositiveOffsetPending || isResyncedHold) && slots.length > 0) {
      // The first slot is synthetic (current held state ending at scheduledAutoTransitionIso,
      // or the community/user-resynced current state)
      return 0;
    }
  ...
  return (
    <View style={tlStyles.card}>
      ...
      {displaySlots.map((slot, i) => {
        ...
        // For the active held slot — POSITIVE_OFFSET_PENDING or community/user-resynced —
        // use anchorStartIso for start time, since it carries the corrected real start moment.
        let currentStartF: string;
        if (isActive && (isPositiveOffsetPending || isResyncedHold) && anchorStartIso) {
        ...
        const startF = isActive && (isPositiveOffsetPending || isResyncedHold) && anchorStartIso
          ? currentStartF  // always use fresh anchor for active pending/resynced slot
          : (stableStartMapRef.current[slotKey] ?? currentStartF);
