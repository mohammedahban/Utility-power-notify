...
function CommunityActivity({ pendingAlerts, onViewAll, userId, onReporterPress }: {
  ...
  return (
    <View style={caStyles.card}>
      ...
      {/* TMMS V2.1: Only UTILITY_ON reports are created by users; filter list
          to show ON reports first. Any legacy UTILITY_OFF entries in the DB
          are shown with a muted "تلقائي" badge to clarify they pre-date V2.1. */}
        ...
        return (
          <View key={r.id} style={caStyles.reportRow}>
            <View style={caStyles.reportMeta}>
              {/* TMMS V2.1: UTILITY_OFF entries are from before the migration */}
              {!isOn && (
                <Text style={[caStyles.timeAgo, { color: T.textMuted }]}>سابق</Text>
              )}
              {isOn && r.yesCount > 0 && <Text style={caStyles.yesCount}>✓ {r.yesCount} موافقة</Text>}
            ...
            <View style={caStyles.reportLeft}>
              ...
              {isOn ? (
                ...
                // TMMS V2.1: Legacy UTILITY_OFF reports are not interactable
                <Text style={[caStyles.reportUser, { color: T.textMuted }]}>سجل قديم</Text>
              )}
            </View>
          </View>
        );
  ...
  return (
    <View style={pnStyles.banner}>
      <View style={{ flex: 1 }}>
        ...
        {/* TMMS V2.1: copy is ON-specific — users only report electricity returning ON */}
        <Text style={pnStyles.body}>
          لم تُبلّغ عن تشغيل الكهرباء منذ فترة. عند عودة الكهرباء في حيّك — اضغط{' '}
          <Text style={{ fontWeight: '800', color: T.accent }}>"الإبلاغ عن تشغيل"</Text>{' '}لمزامنة جدولك ومساعدة جيرانك. 🎯
        </Text>
      </View>
