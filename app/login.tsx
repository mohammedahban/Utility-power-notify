...
export default function LoginScreen() {
  ...
  // NOTE: The auto-redirect effect that used to live here has been
  // removed intentionally.
  //
  // Previously this screen watched `session` and, once it became non-null,
  // redirected to /(tabs). That caused two problems:
  //   1. It raced with AuthGate, which redirects to /(user) or /(admin)
  //      based on role — so the user could be bounced between routes.
  //   2. On cold start, if the session recovered *after* AuthGate had
  //      already sent the user to /login, this effect would fire and
  //      redirect to /(tabs), but only if the user happened to still be
  //      on /login — which was itself the bug being reported.
  //
  // AuthGate in _layout.tsx now owns ALL post-auth routing. After a
  // successful signIn(), the SIGNED_IN event flows through AuthContext →
  // AuthGate, which routes the user to the correct home screen based on
  // their role. No screen-level redirect is needed.
