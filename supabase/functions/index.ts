// analyze-patterns Edge Function — APPPE v4.2
...
// v4.2 — Drift computation result with new diagnostic fields
interface DriftResult {
  offsetMin: number;
  sampleCount: number;
  quantizationRatio: number;  // fraction of samples equal to the dominant value
  dominantError: number;      // the most common error value (min)
}
...
function detectConsecutiveErrorCrisis(historyNewestFirst: HistoryRow[]): CrisisResult {
  ...
  const allLate = lastThree.every((h) => h.errorMinutes > 15);
  const allEarly = lastThree.every((h) => h.errorMinutes < -15);

  if (!allLate && !allEarly) {
    return { active: false, reason: null, offShift: 0, onShift: 0 };
  }

  // v4.2 NOTE: reason text reflects that NO duration shift is applied —
  // only confidence reduction and range widening. Previous versions
  // described a "center shift" that never actually happened in v4.1+.
  const directionAr = allLate ? "متأخرة" : "مبكرة";
  return {
    active: true,
...
function computeDriftOffset(historyNewestFirst: HistoryRow[]): DriftResult {
  ...
  if (slice.length === 0) {
    return { offsetMin: 0, sampleCount: 0, quantizationRatio: 0, dominantError: 0 };
  }
  ...
  // v4.2 — Quantization detector. If >50% of errors are identical, the
  // median is likely a polling-resolution artifact, not real drift. Cap
  // the offset magnitude at half the dominant error value to avoid
  // over-correcting.
...
Deno.serve(async (req) => {
  ...
  if (cycles.length === 0) {
    ...
    // v4.2 — Surface quantization cap when it actively reduced the offset
    if (driftResult.quantizationRatio > DRIFT_QUANTIZATION_THRESHOLD && Math.abs(driftResult.dominantError) > 0) {
      reasoning.push(`Drift correction capped at ${fmtMin(Math.abs(driftResult.offsetMin))} — ${Math.round(driftResult.quantizationRatio * 100)}% of recent errors clustered on ${fmtSignedMin(driftResult.dominantError)} (likely polling-resolution artifact), so the offset was halved to avoid over-correction.`);
    }
