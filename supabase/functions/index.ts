...
function detectConsecutiveErrorCrisis(historyNewestFirst: HistoryRow[]): CrisisResult {
  ...
  return {
    active: true,
    reason: `آخر 3 توقعات كانت جميعها ${directionAr} — تم تطبيق تصحيح الانحراف ${fmtSignedMin(Math.round(ema))}. نطاق التوقع موسّع تلقائياً.`,
...
Deno.serve(async (req) => {
  ...
  const isUnstable = stabilityRaw < 0.28 || volatilityEMA >= 70;
